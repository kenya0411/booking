...
class MTSSB_Mail {
	...
	private function _booking_content($booking, $template) {
		...
		$body = apply_filters('booking_form_number_title', '[ご予約]', 'mail') . "\n"
		 // . "{$article['name']}\n"
		 . apply_filters('booking_form_date_title', '日時：') . apply_filters('booking_form_date', date('Y年n月j日', $booking['booking_time']), $booking['booking_time']) ."\n";
		...
		// $body .= "\n";
		// if (0 < $column['vehicle']) {
			$body .= apply_filters('booking_form_vehicle', '車両') . "：{$client['vehicle']}\n";
		// }	
