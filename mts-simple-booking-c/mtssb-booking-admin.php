...
class MTSSB_Booking_Admin extends MTSSB_Booking {
	...
	public function booking_page() {
			...
			switch ($action) {
				...
				if ($booking_id) {
					...
					$this->booking = $this->new_booking($this->booking['booking_time'] - $this->booking['booking_time'] % 86400, $this->booking['article_id']);
						// $this->booking = $this->new_booking($this->booking['booking_time'] - $this->booking['booking_time'], $this->booking['article_id']);
		...
		<div class="wrap columns-2">
			...
			echo '<a href="?page='."simple-booking&amp;bid={$this->booking['booking_id']}\">";
			echo "予約詳細へ戻る【{$this->booking['client']['name']}】</a>";
				...
				<table class="form-table" style="width: 100%">
					...
					<tr class="form-field">
						...
						<td>
							<?php 
							global $wpdb;
		// $sql = "SELECT booking_time FROM wp_mtssb_booking WHERE booking_time = 1577268000";

		// // var_dump($sql);
		// return $sql;
		// echo $sql;
/*
 * クーポンコードを利用した顧客の一覧を取得
 */
// function get_users_by_coupon($coupon_code) {
  // $wpdbクラスを呼び出す
//   global $wpdb;
//   // SQL文を書く
//   $sql = "SELECT booking_time
//     FROM {$wpdb->prefix}wp_mtssb_booking
//     WHERE booking_time = '1577786400'";
//   // クーポンコードをプリペアドステートメントに渡す
//   $prepared  = $wpdb->prepare($sql, esc_sql($coupon_code));
//   // 受注IDの一覧を取得
//   $order_ids = $wpdb->get_col($prepared);
//   // 受注データから顧客IDを取得し、配列に格納
//   $users = [];
//   foreach ($order_ids as $order_id) {
//     // $users[] = get_post_meta($order_id, '_customer_user', true);
//     echo  $order_id;
//   }
//   // 顧客IDの一覧を返す
//   // return $users;
...
global $wpdb, $mts_simple_booking;
		$data = $wpdb->get_results($wpdb->prepare("
			...
			WHERE booking_time=%d
			ORDER BY booking_id DESC", self::USER_ADJUSTED), ARRAY_A);
...
// var_dump($wpdb);
var_dump($wpdb);
var_dump( $mts_simple_booking);
// 
echo "</pre>";

							 ?>
							<select id="booking-article" class="booking-select-article" name="booking[article_id]" style="display: none">
							...
							<select id="booking-time" class="booking-select-time" name="booking[timetable]">
								...
								if (empty($this->articles[$article_id]['timetable'])) {
									...
									foreach ($this->articles[$article_id]['timetable'] as $time) {
										echo "<option data-count=\"$hogehoge\" value=\"$time\"" . ($timetable == $time ? ' selected="selected"' : '') . ">" .$vehicleTimeTable[$time] . "</option>\n";
									}
								} ?>
							</select>
							<script>	
								jQuery(function($){
									...
									$(function(){
							//車両の初期選択
							var data = $('#booking-time option').data('count')
							$('#booking-time [value="'+data+'"]').prop('selected', true)
								});
								});
