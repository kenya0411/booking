 ...
 */
 jQuery(document).ready(function($) {
 	...
 	//全てにチェックを入れる
 	$('#schedule-check-all').on('change',function(){
 		if ($(this).get(0).checked) {
 			$('.mtssb-schedule input').attr('checked', 'checked').parent().addClass('open')
 		}
 		else {
 			$('.mtssb-schedule input').removeAttr('checked').parent().removeClass('open');
			// $(this).parent().removeClass('open');
		}
	});
 });
