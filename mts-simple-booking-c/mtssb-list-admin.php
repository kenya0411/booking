		...
		<div id="schedule-select-article">
			...
			<form method="get" action="">
				...
				<select class="select-year" name="year">
					<?php for ($y = $this_year - 1; $y <= $this_year + 2; $y++) {
						echo "<option value=\"$y\"";
						if ($y == $this_year) {
				...
				</select><span class="selectYM">年</span>

				<?php 
				// _e('Month:',$this->domain); 
				?>
				...
				</select><span class="selectYM">月</span>
...
class MTSSB_Booking_List extends WP_List_Table {
	...
	public function column_default($item, $column_name) {
					//車両の追加コード
					global $vehicleTimeTableList; 
					global $vehicleTimeTable; 

					 $hoge = date('H', $item['booking_time']);
		...
		switch ($column_name) {
			...
			return $item[$column_name];
			case 'vehicle' :
			return $vehicleTimeTable[$hogehoge];
			case 'created' :
