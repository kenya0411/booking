				...
				// 行終了
					...
					if (0 < $day && $day <= $days) {
						...
						// echo '<a href="?page=' . self::PAGE_NAME . '&amp;dt=' . mktime(0, 0, 0, $themonth, $day, $theyear) . "\">$viewstr</a>";
							echo '<a href="?page=' . self::PAGE_NAME . '&amp;dt=' . mktime(0, 0, 0, $themonth, $day, $theyear) . "\">予約の確認</a>";
		...
		foreach ($this->articles as $article_id => $article) {
			...
			if (!empty($this->schedule[$article_id]) && $this->schedule[$article_id][$day]['open'] == 1) {
				...
				if (isset($this->reserved[$thetime][$article_id])) {
					...
					echo  $this->reserved[$thetime][$article_id]['number'].'台';
					echo '</strong>';
					
				} else {
					echo '<span class="gray">0</span>';
				}
			} else {
	...
	public function day_page($daytime) {
		...
		<div class="wrap">
			...
			<?php if (!empty($this->message)) : ?><div class="<?php echo $this->errflg ? 'error' : 'updated' ?>">
				...
				<?php if ($this->today_time <= $daytime) : ?><p><?php 
			// _e('The number of booking can be adjusted by setting a number to reduce to a text box.', $this->domain) 
					?></p><?php endif; ?>
	...
	protected function _day_article($article_id, $daytime, $nonce) {
		...
		<div class="article-each">
			...
			<div class="article-time">
				...
				<div class="booking-able">
					予約可能の有無
				</div>
				<div class="booking-each">
					予約名
				</div>
			...
			<form method="post" action="<?php echo '?page=' . self::PAGE_NAME . "&amp;dt={$daytime}" ?>" name="article_<?php echo $article_id ?>">
				...
				// 予約数をcapacity,quantity別で計算する
					...
					// echo "<input type=\"text\" class=\"small-text\" name=\"booking[booking_time][$thetime]\" value=\"" . (empty($adjustment) ? '0' : ($capacity ? $adjustment[0]['number'] : count($adjustment))) . "\" />";
							echo "<select name=\"booking[booking_time][$thetime]\">";
							echo "<option  value=\"" . (empty($adjustment) ? '0' : ($capacity ? $adjustment[0]['number'] : count($adjustment))) . "\" /></option>";
						 if ( (empty($adjustment) ? '0' : ($capacity ? $adjustment[0]['number'] : count($adjustment)))  === '0' && empty($reserved) ) {
							echo '可能';
							echo '<option value="1" />これ以上予約を受け付けない</option>';


						}else if((empty($adjustment) ? '0' : ($capacity ? $adjustment[0]['number'] : count($adjustment)))  === 1 && empty($reserved) ){
							echo '<option value="0" />予約を受けつける</option>';
					...
					<div class="booking-able">
						...
						else if((empty($adjustment) ? '0' : ($capacity ? $adjustment[0]['number'] : count($adjustment)))  === 1){
							echo "不可";
...
else if ( (empty($adjustment) ? '0' : ($capacity ? $adjustment[0]['number'] : count($adjustment)))  === '0' && !empty($reserved) ) {
							echo '可能';
...
<!-- 			<tr>
			...
			<tr>
				...
				<td><?php echo $datestr
				// . date(' H:i', $this->booking['booking_time']) 
				?></td>
			</tr>
			...
			<tr>
				...
				<td>
					...
					global $vehicleTimeTableList,$vehicleTimeTable;
					$hoge = date('H', $this->booking['booking_time']);
					$hogehoge = $vehicleTimeTableList[$hoge];
					echo $vehicleTimeTable[$hogehoge];
					?>
				</td>
