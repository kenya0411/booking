				...
				<fieldset id="booking-reservation-fieldset">
					...
					<table>
						<tr>
							<th>予約</th>
							<td>
								<?php echo apply_filters('booking_form_date', date('Y年n月j日', $this->thetime), $this->thetime) ?>
							</td>
						</tr>
									<?php 
									// 車両追加分

									if (0 < $this->clcols['vehicle']) : ?>
										<tr>
										<th><label for="client-vehicle"><?php echo apply_filters('booking_form_vehicle', '車両'); echo $this->clcols['vehicle'] == 1 ? $this->_require_message() : '' ?></label></th>
										<td>
											...
											<script>
												...
												jQuery(function($){
												$(function(){
													var data = $('[data-name="vehicleCheck"]').data('check')
													$('[data-count="'+data+'"]').prop('checked', true);
											...
											<?php
											global $vehicleTimeTableList,$vehicleTimeTable; 
											$vehicleCount = date('H', $this->thetime);
											foreach($vehicleTimeTableList as $key => $val){

												?>
												<input style="display: none;" type='radio' name="booking[client][vehicle]" value="<?php echo $vehicleTimeTable[$val] ?>" id="vehicle<?php echo $val ?>" data-name="vehicleCheck" data-count="<?php echo $key ?>" data-check="<?php echo $vehicleCount;?>" >
												<label style="display: none;" for="vehicle<?php echo $key ?>"><?php echo $vehicleTimeTable[$val] ?></label>
											<?php } ?>
<?php echo $vehicleTimeTable[$vehicleTimeTableList[$vehicleCount]]; ?>
											<?php if (isset($this->errmsg['vehicle'])) : ?><div class="error-message"><?php echo $this->errmsg['vehicle'] ?></div><?php endif; ?></td>
											</tr><?php endif; //車両ここまで?>
				...
				<fieldset id="booking-confirm-fieldset">
					...
					<table>
						<tr>
							<th>予約</th>
							<td><?php 
							// echo esc_html($this->article['name']) 
							?>
								<?php echo apply_filters('booking_form_date', date('Y年n月j日', $this->booking['booking_time']), $this->booking['booking_time']) ?>
							</td>
						</tr>
								<?php if (0 < $this->clcols['vehicle']) : ?><tr>
									<th><?php echo apply_filters('booking_form_vehicle', '車両') ?></th>
									<td>
										<?php echo esc_html($client['vehicle']) ?>
										<input type="hidden" name="booking[client][vehicle]" value="<?php echo esc_html($client['vehicle']) ?>" />
									</td>
									</tr><?php endif; ?>
